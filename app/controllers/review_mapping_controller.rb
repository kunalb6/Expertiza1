class ReviewMappingController < ApplicationController
  ...
  def distribution
    ...
    teams.each do |team|
      team_score=0.0
      team_score_count = 0.0
      response_maps = ResponseMap.where( ["reviewee_id = ? and type = ?",team.id,  objtype])
      ...
      if(team_score_count>0)
       ...
       # if (@scores[(team_score/10-1).ceil.to_i] > max_score)
        #  max_score = @scores[(team_score/10).ceil.to_i]
        #end
      end
    end
